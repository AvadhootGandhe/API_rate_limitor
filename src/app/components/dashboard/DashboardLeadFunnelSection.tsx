...
export function DashboardLeadFunnelSection() {
  ...
  return (
    <section id="lead-funnel" className="space-y-6 scroll-mt-24">
      <div>
        <h2 className="text-xl font-semibold text-gray-900">Lead funnel</h2>
        <p className="text-sm text-gray-600 mt-1">Stages, counts, and horizontal funnel chart.</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
        ...
        <div className="space-y-4">
          {data.funnel.map(
            (stage: { stage: keyof typeof STAGE_ICONS; count: number; percentage: number }, index: number) => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {data.funnel.map((stage: { stage: keyof typeof STAGE_ICONS; count: number; percentage: number }, index: number) => {
          const Icon = STAGE_ICONS[stage.stage];
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-3xl font-semibold text-gray-900 mb-2">{stage.count.toLocaleString()}</p>
                  <div className="flex items-center gap-1">
                    <span className="text-xs font-medium text-gray-600">{stage.percentage}% of total</span>
                  </div>
                </div>
                <div
                  className="w-12 h-12 rounded-lg flex items-center justify-center"
        ...
        <ResponsiveContainer width="100%" height={350}>
          <BarChart data={data.funnel} layout="vertical">
            ...
            <Bar dataKey="count" radius={[0, 8, 8, 0]}>
              {data.funnel.map((entry: { stage: string }, index: number) => (
